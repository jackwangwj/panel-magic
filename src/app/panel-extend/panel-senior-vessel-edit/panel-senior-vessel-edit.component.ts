...
export class PanelSeniorVesselEditComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.currentStatusRX$ = this.panelSeniorVesselEditService.launchCurrentStatusIdChange$.pipe(
			debounceTime(1)
		).subscribe(status=>{
			const _current_status = this.panelSeniorVesselEditService.currentStatusInVesselInfo;
			if (_current_status && this.isEnterEditVesselCondition ) {
				this.panelScopeEnchantmentService.scopeEnchantmentModel.emptyAllProfile();
				this.panelExtendService.nextWidgetList(this.panelExtendService.handleFreeItemToPanelWidget(_current_status.widgetList))
				// 将该动态容器的状态的数据记录在DB里（不同状态开辟一张表）
				this.panelExtendMoveBackService.createCollections(_current_status.uniqueId).subscribe(_b => { if (_b) this.panelExtendService.launchSaveIndexedDB$.next() });
			}
		})
	}
	...
	public acceptExitRoomVessel(): void {
		...
		this.panelExtendMoveBackService.createCollections(this.appDataService.currentAppDataForinPageData.router).subscribe(_b => { if (_b) this.panelExtendService.launchSaveIndexedDB$.next() });
	}
	...
	public saveVesselWidgetData(): void {
		...
		if( _vessel_widget ) {
			_current_status.widgetList = this.panelExtendService.handleSaveWidgetToOrientationModelData(this.riverDiversionWidgetList)
			this.currentEditVesselWidget.autoWidget.content.vesselWidget = cloneDeep(this.panelSeniorVesselEditService.vesselWidgetModel$.value)
	...
	public handleNoneStatusListInit(): void {
		const _status_list = get(this.currentEditVesselWidget, 'autoWidget.content.vesselWidget.statusWarehouseCollection')
