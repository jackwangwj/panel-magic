...
import { CombinationWidgetModel } from "./CombinationWidgetModel";
import { BaseValueChangeClass } from "app/baseClass";

export interface IFourBearingProportion { left: number, top: number, right: number, bottom: number };

export interface IProfileable<T> { left: number, top: number, width: number, height: number, rotate: number, opacity: number, zIndex: number, valueChange$: Subject<T> };
...
export class ProfileModel extends BaseValueChangeClass<ProfileModel> implements IProfileable<ProfileModel> {
	 ...
	 */
	public readonly combinationWidgetData$: BehaviorSubject<CombinationWidgetModel> = new BehaviorSubject<CombinationWidgetModel>(null)

	// 单位： px | %
	public get unit() : 'px' | '%' { return this._unit }
		...
		if (data) {
			for (let e in this) {
				if ((<Object>data).hasOwnProperty(e)) {
					this['_'+e] = (<any>data)[e];
					if (e == 'width' || e == 'height') {
						this['_'+e] = this['_'+e] < (<any>10) ? (<any>10) : this['_'+e]
					};
				}
			};
		}
		...
		if( data ) {
			for (let e in this) {
				if ((<Object>data).hasOwnProperty(e)) {
					this[e] = (<any>data)[e];
					if( e == 'width' || e == 'height' ) {
						this[e] = this[e] < (<any>10) ? (<any>10) : this[e]
					};
				}
			};
