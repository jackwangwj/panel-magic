...
// clip-path拖拽笔触的类型
export type TClipPointType = 'inset-c'
	| 'inset-l' | 'inset-t' | 'inset-r' | 'inset-b' | 'circle-c' | 'circle-side'
	| 'ellipse-c' | 'ellipse-r' | 'ellipse-b' | 'polygon' | ''
/**
 * 多边形的类型
...
export class ClipInsetModel {
	...
	public get styleContent() : {[key: string]: string} {
		return { 'clip-path': `inset(${this.insetT.top}% ${100 - this.insetR.left}% ${100 - this.insetB.top}% ${this.insetL.left}%)` }
	}
}
...
export class ClipCircleModel {
	...
	public calcSlopeK(pro: ProfileModel): number {
		const _ = (pro.height - ((this.circleC.top / 100) * pro.height) - pro.height / 2) / (((this.circleC.left / 100) * pro.width) - pro.width / 2)
		return isNaN(_) ? 1 : _
	}
	// 计算斜率公式的b
	public calcSlopeB(pro: ProfileModel): number {
		return pro.height - (this.circleC.top / 100) * pro.height - this.calcSlopeK(pro) * ((this.circleC.left / 100) * pro.width)
	}
	// 根据side圆周上的坐标计算半径
	...
	public handleCalcCircleSidePoint( pro: ProfileModel ): void {
		// 百分比转化为常值
		let _calc_per = (n: number, type: 'width' | 'height' = 'width'): number=>{ return (n / 100) * pro[type] }
		// 计算半径
		let _calc_radius = (pro.width * (this.circleRadius / 100) + pro.height * (this.circleRadius / 100)) / 2
		// 计算圆中心点离轮廓中心点的距离
		let _calc_center_dist = ()=>{
			let _ = Math.sqrt((_calc_per(this.circleC.left) - pro.width / 2) ** 2 + (_calc_per(this.circleC.top, 'height') - pro.height / 2) ** 2)
			return _ == 0 ? 1 : _
		}
		/**
		 * 进而得出公式
		 ...
		 * x=(y-b)/k
		 */
		this.circleSide.left = ((_calc_center_dist() - _calc_radius) / _calc_center_dist()) * (_calc_per(this.circleC.left) - pro.width / 2) + pro.width / 2
...
export class ClipEllipseModel {
	...
	public get styleContent(): { [key: string]: string } {
		return { 'clip-path': `ellipse(${Math.abs(this.ellR.left - this.ellC.left)}% ${Math.abs(this.ellB.top - this.ellC.top)}% at ${this.ellC.left}% ${this.ellC.top}%)` }
	}
}
...
export class ClipPolygonModel {
	...
	// 拖拽点的订阅值
	public get pointArr(): Array<ClipPointModel> { return this.pointList$.value };

	constructor() { }

	public get styleContent(): { [key: string]: string } {
		if (this.pointArr.length == 3) {
			this.styleStr = `${this.pointArr[0].left}% ${this.pointArr[0].top}%, ${this.pointArr[1].left}% ${this.pointArr[1].top}%, ${this.pointArr[2].left}% ${this.pointArr[2].top}%`
		}else if(this.pointArr.length == 4) {
			this.styleStr = `${this.pointArr[0].left}% ${this.pointArr[0].top}%, ${this.pointArr[1].left}% ${this.pointArr[1].top}%, ${this.pointArr[2].left}% ${this.pointArr[2].top}%, ${this.pointArr[3].left}% ${this.pointArr[3].top}%`
		}else if(this.pointArr.length == 5) {
			this.styleStr = `${this.pointArr[0].left}% ${this.pointArr[0].top}%, ${this.pointArr[1].left}% ${this.pointArr[1].top}%, ${this.pointArr[2].left}% ${this.pointArr[2].top}%, ${this.pointArr[3].left}% ${this.pointArr[3].top}%, ${this.pointArr[4].left}% ${this.pointArr[4].top}%`
		}else if(this.pointArr.length == 6) {
			this.styleStr = `${this.pointArr[0].left}% ${this.pointArr[0].top}%, ${this.pointArr[1].left}% ${this.pointArr[1].top}%, ${this.pointArr[2].left}% ${this.pointArr[2].top}%, ${this.pointArr[3].left}% ${this.pointArr[3].top}%, ${this.pointArr[4].left}% ${this.pointArr[4].top}%, ${this.pointArr[5].left}% ${this.pointArr[5].top}%`
		};
