...
import { ScopeEnchantmentModel, ProfileModel, AuxliLineModel, CornerPinModel, ECursor, PanelScopeTextEditorModel, OuterSphereHasAuxlModel, ClipPathMaskModel } from './Model';
...
import { debounceTime, map, last, distinctUntilChanged } from 'rxjs/operators';
...
@Component({
	...
	styleUrls: [
		'./panel-scope-enchantment.component.scss',
		'./panel-scope-corner-pin.scss'
	]
})
export class PanelScopeEnchantmentComponent implements OnInit, OnDestroy {
	@ViewChild('widgetContextMenuEl') public widgetContextMenuEl: TemplateRef<any>;
	@ViewChild('panelScopeTextEditorComponentEl') public panelScopeTextEditorComponentEl: PanelScopeTextEditorComponent;
	// 组件的右键事件
	private widgetContextMenu: NzDropdownContextComponent;
	...
	constructor(
		public clipPathService: ClipPathResizeMaskService,
		...
		this.mouseContextMenu$ = this.panelScopeEnchantmentService.launchContextmenu$.subscribe(event=>{
			// 生成右键菜单
			if( event ) this.widgetContextMenu = this.nzDropdownService.create(event, this.widgetContextMenuEl);
		})

		this.mouseIntcrementRX$ = this.panelScopeEnchantmentService.launchMouseIncrement$
			.pipe().subscribe((drag: DraggablePort) => {
				if (drag) { this.handleProfileOuterSphereLocation(drag) };
			});
		...
		// 剪贴蒙版的订阅
		this.clipPathRX$ = this.clipPathMask.currentPathType$.subscribe(type=>{
	...
	public openRotateSubject( proOuter: OuterSphereHasAuxlModel ): void {
		setTimeout(() => { this.scopeEnchantment.handleCursorPinStyle(this.panelScopeEnchantmentService.conversionRotateOneCircle(proOuter.rotate))});
		if (this.profileOuterSphereRotateRX$ ) this.profileOuterSphereRotateRX$.unsubscribe();
		this.profileOuterSphereRotateRX$ = proOuter.valueChange$.pipe(
			map(_v=>_v.rotate),
			debounceTime(1),
	 ...
	 * 右键菜单关闭事件
	 */
	public contextMenuClose(): void {
		this.widgetContextMenu.close();
	}
	...
	public handleRectInsetWidget( port: ILocation ): void {
		...
		if( port.width != 0 && port.height != 0 ) {
			_all_widget.forEach(_w=>{
				...
				if( _w.profileModel.rotate != 0 ) _offset_coord = this.panelScopeEnchantmentService.handleOuterSphereRotateOffsetCoord(_w.profileModel);
	...
	public createAllLineSave(): void {
		...
		const _panel_info = this.panelExtendService.panelInfoModel
		const _fn_offset = this.panelScopeEnchantmentService.handleOuterSphereRotateOffsetCoord.bind(this.panelScopeEnchantmentService)
	...
	public acceptRotateIco( mouse: MouseEvent ): void {
		...
		_mouse_move$ = fromEvent(document, 'mousemove').subscribe((move: MouseEvent) => {
			this.zone.run(() => {
				// 记录轮廓中心坐标点
				const _pro_center_coor = [_pro.left + _pro.width / 2 + _panel_info.left, _pro.top + _pro.height / 2 + _panel_info.top];
