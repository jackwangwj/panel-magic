...
export class PanelScopeTextEditorComponent implements OnInit {
	@ViewChild('textEditorEl') public textEditorEl: ElementRef
	@ViewChild('spanEl') public spanEl: ElementRef
	...
	constructor(
		...
		this.panelTextEditorRX$ = this.panelScopeEnchantmentService.panelScopeTextEditorModel$.pipe()
			.subscribe(value => {
				if (value) {
					// 取消八个拖拽点
		...
		this.keyboardEnterRX$ = fromEvent(document, 'keydown').subscribe((event: KeyboardEvent) => {
			this.zone.run(() => {
				if (event.keyCode == 13) {
					...
					// 判断文字是否超出组件宽度
					setTimeout(() => { this.handleInseWidgetTextWidthLength() });
				}
			})
		})
	}
	...
	public handleInseWidgetTextWidthLength(): void {
		...
		const _widget_width = this.insetWidget.conventionSiteModel.width - (_is_has_border ? this.insetWidget.panelFacadeModel.borderNumber * 2 : 0)
