...
import { ScopeEnchantmentModel, AuxliLineModel, PanelScopeTextEditorModel, ProfileModel, OuterSphereHasAuxlModel, ClipPathMaskModel }	from './Model'
...
export class PanelScopeEnchantmentService {
	...
	// 文字编辑器模式数据模型
	public panelScopeTextEditorModel$: BehaviorSubject<PanelScopeTextEditorModel> = new BehaviorSubject<PanelScopeTextEditorModel>(null)
	// 存储所有辅助线的数据模型
	 ...
	 * isLaunch参数表示是否发送数据源
	 */
	public handleFromWidgetListToProfileOuterSphere( arg: {
		isLaunch?: boolean
	} = {
		isLaunch: true
	} ): void {
	...
	public calcProfileOuterSphereInfo(): OuterSphereHasAuxlModel {
		...
		_inset_widget.forEach(_e => {
			...
			if (_e.profileModel.rotate != 0 && _inset_widget.length > 1) _offset_coord = this.handleOuterSphereRotateOffsetCoord(_e.profileModel);
			...
			_min_width_empty 	= _e.profileModel.left + _e.profileModel.width < 0 ? Math.min(_min_width_empty, Math.abs(_e.profileModel.left) - _e.profileModel.width) : 0
			_min_height_empty 	= _e.profileModel.top + _e.profileModel.height < 0 ? Math.min(_min_height_empty, Math.abs(_e.profileModel.top) - _e.profileModel.height) : 0
		});
	 ...
	 */
	public handleOuterSphereRotateOffsetCoord( arg: ProfileModel, type: 'lt' | 'rt' | 'lb' | 'rb' = 'lt' ): { left: number, top: number} {
	 ...
	 */
	public handleAuxlineCalculate(target: OuterSphereHasAuxlModel = this.scopeEnchantmentModel.valueProfileOuterSphere): void {
		...
		if (_mouse_coord) {
			for (let i: number = 0, l: number = _aux.vLineList.length; i < l; i++) {
				if (Math.abs(_aux.vLineList[i] - _mouse_coord[0] + _offset_amount.left * -1) <= 4) {
					_outer_sphere.left = _aux.vLineList[i] + _offset_amount.left * -1
					_outer_sphere.lLine = true
				}
				if (Math.abs(_aux.vLineList[i] - (_mouse_coord[0] + _outer_sphere.width) + _offset_amount.left) <= 4) {
			...
			for (let i: number = 0, l: number = _aux.hLineList.length; i < l; i++) {
				if (Math.abs(_aux.hLineList[i] - _mouse_coord[1] + _offset_amount.top * -1) <= 4) {
					_outer_sphere.top = _aux.hLineList[i] + _offset_amount.top * -1
					_outer_sphere.tLine = true
				}
				if (Math.abs(_aux.hLineList[i] - (_mouse_coord[1] + _outer_sphere.height) + _offset_amount.top) <= 4) {
			...
			for (let i: number = 0, l: number = _aux.hcLineList.length; i < l; i++) {
				if (Math.abs(_aux.hcLineList[i] - (_mouse_coord[1] + _outer_sphere.height/2)) <= 4) {
			...
			for (let i: number = 0, l: number = _aux.vcLineList.length; i < l; i++) {
				if (Math.abs(_aux.vcLineList[i] - (_mouse_coord[0] + _outer_sphere.width / 2)) <= 4) {
	...
	public conversionTwoCoordToRotate( coord: [number, number] ): number {
		if( Array.isArray( coord ) && coord.length == 2 ) {
			if ( coord[0] >= 0 && coord[1] > 0 ) {
				return this.conversionRotateFromRadian(coord[0] / coord[1])
			} else if ( coord[0] > 0 && coord[1] <= 0 ) {
				return this.conversionRotateFromRadian(coord[1] / coord[0] * -1) + 90
			} else if ( coord[0] <= 0 && coord[1] < 0 ) {
				return this.conversionRotateFromRadian(coord[0] / coord[1]) + 180
			} else if ( coord[0] < 0 && coord[1] >= 0 ) {
				return this.conversionRotateFromRadian(coord[1] / coord[0] * -1) + 270
			}
		};
		return;
	}
	...
	public conversionRotateToQuadrant( rotate: number ): number {
		...
		const _qua_obj = {
			1: rotate >= 0 && rotate < 90,
			4: rotate >= 90 && rotate < 180,
			3: rotate >= 180 && rotate < 270,
			2: rotate >= 270 && rotate < 360
		};
		for( let e in _qua_obj ) {
			if( _qua_obj[e] == true ) {
				return (<any>e) * 1
			}
		};
	...
	public conversionRotateNewCoordinates( coord: [number, number], rotate: number ): { left: number, top: number } {
		...
		const _offset_coord = this.conversionRotateToOffsetLeftTop(
			<ProfileModel>{
