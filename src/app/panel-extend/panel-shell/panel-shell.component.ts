...
export class PanelShellComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		private navigationBarViewService: NavigationBarViewService,
		...
		this.appDataRX$ = this.appDataService.launchAppData$.subscribe(app=>{
			...
			if( app ) {
				const _tabbar_widget = app.app_config && app.app_config.tabbar_widget;
				if (_tabbar_widget ) {
					const _tabbar_model = _tabbar_widget.content && _tabbar_widget.content.tabbarModel;
					if( _tabbar_model ) {
				...
				this.appDataService.setAppConfigData('tabbar_widget', this.tabbarModel.autoWidget)
			}
		})
	}
	...
	public handleCalcTrackData(drag: DraggablePort): void {
		...
		const _panel_top_bottom_height = Math.abs(this.panelInfo.top) + this.panelInfo.height + Math.abs(this.panelInfo.bottom)
