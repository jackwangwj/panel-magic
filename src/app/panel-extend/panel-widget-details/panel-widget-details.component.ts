import { Component, OnInit, ComponentRef, ViewContainerRef, ViewChild, ComponentFactoryResolver, OnDestroy } from '@angular/core';
...
export class PanelWidgetDetailsSiteComponent implements OnInit {
	...
	@ViewChild('widgetSiteContainer', { read: ViewContainerRef })
	...
	constructor(
		...
		this.outerSphereInsetWidgetRX$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.outerSphereInsetWidgetList$.subscribe(value=>{
	 ...
	 * 创建部件自己的设置项目
	 */
	public createSiteComponent(type: string, widget: PanelWidgetModel = this.panelScopeEnchantmentService.scopeEnchantmentModel.outerSphereInsetWidgetList$.value[0]): void {
		if( allPanelWidgetSiteObj[type] ) {
			...
			this.currentComponentRef = this.widgetSiteContainer.createComponent(this.componentFactoryResolver.resolveComponentFactory(allPanelWidgetSiteObj[type]))
