...
@Component({
	selector: 'app-panel-details-list-site',
	template: ``,
	styleUrls: ['../ant-collapse.scss']
})
export class PanelDetailsListSiteComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.profileOutershpere$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.outerSphereInsetWidgetList$.subscribe(
			value => {
				if (Array.isArray(value) && value.length == 1 && value.find(_w => _w.type == 'detailslist') && this.panelScopeEnchantmentService.scopeEnchantmentModel.valueProfileOuterSphere) {
