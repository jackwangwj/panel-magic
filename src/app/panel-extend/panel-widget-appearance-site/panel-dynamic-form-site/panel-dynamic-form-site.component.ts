...
@Component({
	selector: 'app-panel-dynamic-form-site',
	template: ``,
	styleUrls: ['../ant-collapse.scss']
})
export class PanelDynamicFormSiteComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.profileOutershpere$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.outerSphereInsetWidgetList$.subscribe(
			value => {
				if (Array.isArray(value) && value.length == 1 && value.find(_w => _w.type == 'dynamicform') && this.panelScopeEnchantmentService.scopeEnchantmentModel.valueProfileOuterSphere) {
