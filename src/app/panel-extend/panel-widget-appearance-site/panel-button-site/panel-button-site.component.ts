...
export class PanelButtonSiteComponent implements OnInit {
	...
	public acceptTextSiteValueChange( value: PanelTextModel ): void {
		if( value ) {
			...
			_widget.addStyleToUltimatelyStyle(value.styleContent(_w_height - (_w_border_style != 'none' ? _w_border_width * 2 : 0)))
	...
	public acceptFacadeValueChange( value: PanelFacadeModel ): void {
		...
		let _all_style = {
			...value.styleContent(),
			'line-height': _text.styleContent( _w_height - (value.borderStyle != 'none' ? value.borderNumber * 2 : 0) )['line-height']
		};
