...
import { HostItemModel } from 'app/panel-extend/panel-widget/Model/HostModel';
import { WidgetModel, ICustomfeature } from 'app/panel-extend/panel-widget/Model/WidgetModel';
...
export class LinkMenuTriggerComponent implements OnInit, OnDestroy {
	...
	@Output()
	public launchEventHandler: Subject<string> = new Subject<string>();

	@Output()
	public launchEventParams: Subject<any> = new Subject<any>();
	...
	public get currentEventTargetEventHandler(): EventModel {
		return this.eventTarget && get(this.eventTarget, 'panelEventHandlerModel') ? (<PanelWidgetModel>this.eventTarget).panelEventHandlerModel : new EventModel()
	}
	...
	constructor(
		...
		this.isVisibleEventModal$ = this.panelEventService.eventSiteModel.isVisibleModal$.pipe(
			debounceTime(1)
		).subscribe(b=>{
