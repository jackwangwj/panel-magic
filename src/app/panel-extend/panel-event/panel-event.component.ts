...
import { EventSiteModel } from './Model/EventSiteModel';
...
import { EventModel } from './EventHandler';
...
export class PanelEventComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		// 仅支持选中一个组件设置链接事件
		this.profileOutershpereRX$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.profileOuterSphere$.subscribe(outer=>{
			...
			if (outer && Array.isArray(_inset_widget) && _inset_widget.length == 1 && (_widget_container_list[_inset_widget[0].type] || _inset_widget[0].type == 'combination')) {
		...
		this.openEventSiteRX$ = this.panelEventService.eventSiteModel.isVisibleModal$.subscribe(b=>{
	...
	public modifyChangeEvent(): void {
		...
		this.eventSiteModel.currentEventModel$.next(this.panelEventService.currentPanelWidgetModel.panelEventHandlerModel)
	}
