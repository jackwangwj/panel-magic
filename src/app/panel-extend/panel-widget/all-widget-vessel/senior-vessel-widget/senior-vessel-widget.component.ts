...
export class SeniorVesselWidgetComponent implements OnInit, OnChanges {
	...
	ngOnChanges( change: SimpleChanges) {
		if (change.widget && change.widget.currentValue ) {
			const _current_status = get(change.widget.currentValue, 'autoWidget.content.vesselWidget.currentStatusId', '');
			const _repertory = get(change.widget.currentValue, 'autoWidget.content.vesselWidget.repertoryStatusWarehouse', '')
			if (_repertory[_current_status] && Array.isArray(_repertory[_current_status].widgetList) ) {
				this.vesselInWidgetList$.next(this.panelExtendService.handleFreeItemToPanelWidget(_repertory[_current_status].widgetList))
			}
		}
	}
