...
export class PanelWidgetComponent implements OnInit, OnChanges {
	...
	public acceptWidgetChecked( event: MouseEvent ): void {
		if (!this.isSimpleFunc ) {
			...
			if (!this.panelScopeEnchantmentService.scopeEnchantmentModel.outerSphereInsetWidgetList$.value.some(_w=>_w.uniqueId == this.widget.uniqueId) ) {
				event.shiftKey == true
	 ...
	 * 双击widget组件
	 */
	public acceptDoubleClick( event: MouseEvent ): void {
		if (!this.isSimpleFunc ) {
			if (this.widget.type == 'text' || this.widget.type == 'button' ) {
				...
				_text_editor.setData({
					...
					textDecoration: _text_widget.lineationType == 'bottom' ? 'underline' : _text_widget.lineationType == 'center' ? 'line-through' : 'initial' ,
