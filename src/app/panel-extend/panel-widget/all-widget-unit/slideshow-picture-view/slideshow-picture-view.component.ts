import { Component, OnInit, Input, OnDestroy, ViewChild, ElementRef, QueryList } from '@angular/core';
...
export class SlideshowPictureViewComponent implements OnInit, OnDestroy {
	@ViewChild('carouselEl') public carouselEl: NzCarouselComponent
	// 订阅值变化
	...
	public set widget(v: PanelWidgetModel) {
		if (v.type != '') {
			...
			this.widgetModel.content.pictureAdModel = new PictureAdModel(
				v.autoWidget.content.pictureAdModel
			)
			...
			this.profileValueChangeRX$ = this.widget.profileModel.valueChange$
				...
				.subscribe(value => {
					if (this.carouselEl ) {
						...
						const _slide_contents: QueryList<NzCarouselContentDirective> = this.carouselEl.slideContents
