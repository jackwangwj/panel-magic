import { ConventionSiteModel, PanelTextModel, PanelFacadeModel, PanelShadowModel, PanelFilterModel, PanelAnimationModel } from "../../panel-widget-appearance/Model";
import { isObject } from "@ng-public/util";
import { ProfileModel } from "../../panel-scope-enchantment/Model";
import { EventModel } from "../../panel-event/EventHandler";
import { HostItemModel } from "./HostModel";
...
export class PanelWidgetModel extends HostItemModel {
	...
	public conventionInit(): void {
		if( get(this.autoWidget, 'orientationmodel' ) && get(this.autoWidget,'style.data') ) {
			const _ori = this.autoWidget.orientationmodel;
			const _opacity = (<Object>this.autoWidget.style.data).hasOwnProperty('opacity') ? this.autoWidget.style.data.opacity * 100 : 100
	...
	public panelTextInit(): void {
		if( get(this.autoWidget, 'orientationmodel' ) ) {
			...
			if (_style.hasOwnProperty('font-weight')) _set_obj['isBold'] = _style['font-weight'] == 'bold' ? true : false;
			if (_style.hasOwnProperty('font-style')) _set_obj['isItalic'] = _style['font-style'] == 'italic' ? true : false;
			...
			if (_style.hasOwnProperty('font-size') && _style.hasOwnProperty('line-height') && _style.hasOwnProperty('height')) {
	...
	public panelFacadeInit(): void {
		if( this.ultimatelyStyle ) {
			...
			if (_style.hasOwnProperty('border-width')) _set_obj['borderNumber'] = _style['border-width'].replace('px', '') * 1;
			if (_style.hasOwnProperty('border-radius') && _style['border-radius']) {
				...
				if( Array.isArray(_radius_arr) ) {
					...
					if( _radius_arr.length == 1 ) { // 四个边的圆角相同
					...
					}else if( _radius_arr.length == 2 ) { // 左上右下 和 右上左下
						_set_obj['ltRadius'] = _radius_arr[0]
						_set_obj['rbRadius'] = _radius_arr[0];
						_set_obj['rtRadius'] = _radius_arr[1];
						_set_obj['lbRadius'] = _radius_arr[1];
					}else if( _radius_arr.length == 3 ) { // 左上 和 右上左下 和右下
						_set_obj['ltRadius'] = _radius_arr[0]
						_set_obj['rtRadius'] = _radius_arr[1]
						_set_obj['lbRadius'] = _radius_arr[1]
						_set_obj['rbRadius'] = _radius_arr[2]
					}else if( _radius_arr.length == 4 ) { // 左上 和 右上 和 右下 和 左下
						_set_obj['ltRadius'] = _radius_arr[0]
						_set_obj['rtRadius'] = _radius_arr[1]
						_set_obj['rbRadius'] = _radius_arr[2]
	...
	public panelAnimationInit(): void {
		if( this.ultimatelyStyle ) {
			const _style = <Object>this.ultimatelyStyle
			let _animation_obj = {}
			if (_style.hasOwnProperty('animation-name') && _style['animation-name']) _animation_obj['animationName'] = _style['animation-name'];
			if (_style.hasOwnProperty('animation-delay') && _style['animation-delay']) _animation_obj['animationDelay'] = _style['animation-delay'].replace('s','');
			...
			if (_style.hasOwnProperty('animation-iteration-count') && _style['animation-iteration-count']) _animation_obj['animationIterationCount'] = _style['animation-iteration-count'];
	...
	public panelEventHandlerInit(): void {
		if (get(this.autoWidget, 'customfeature' ) ) {
			...
			let _event_obj = {}
			if (_event.hasOwnProperty('eventHandler') && _event.eventHandler) { _event_obj['eventHandler'] = _event.eventHandler };
			if (_event.hasOwnProperty('eventParams') && _event.eventParams) { _event_obj['eventParams'] = _event.eventParams };
