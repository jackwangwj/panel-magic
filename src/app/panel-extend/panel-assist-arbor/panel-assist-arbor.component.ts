...
export class PanelAssistArborComponent implements OnInit {
	...
	constructor(
		...
		this.profileOuterSphereRX$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.profileOuterSphere$.subscribe(pro=>{
			if( pro ) {
				this.combinationValueChangeRX$ = pro.valueChange$.pipe(
					debounceTime(1)
				).subscribe(value=>{
		...
		this.temporaryProfileRX$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.profileTemporary$.subscribe(value=>{
			if( value ) {
				const _com = value.combinationWidgetData$.value
	...
	ngOnInit() {
		this.indexedDBDataRX$ = this.panelExtendMoveBackService.launchDBDataValue$.subscribe(res=>{
			...
			if (Array.isArray(_widget_list)) {
				...
				this.panelExtendService.nextWidgetList(this.panelExtendService.handleFreeItemToPanelWidget(_widget_list))
			}
		})
	}
			...
			_child_widget.forEach(_w=>{
				...
				if (_w_com ) {
					_w.profileModel.setData({
						rotate: this.panelScopeEnchantmentService.conversionRotateOneCircle(_w.profileModel.immobilizationData.rotate + _com_pro.rotate),
					})
					/**
					...
					// 根据坐标和角度返回对应的新的坐标
					const _offset_coord = this.panelScopeEnchantmentService.conversionRotateNewCoordinates([_coordinates_x, _coordinates_y], _com_pro.rotate)
					if (_offset_coord ) {
						_w.profileModel.setData({
