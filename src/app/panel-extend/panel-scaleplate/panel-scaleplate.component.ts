import { Component, OnInit, Renderer2, ViewChild, ElementRef, AfterViewInit, NgZone, OnDestroy } from '@angular/core';
...
import { Subscription, BehaviorSubject, merge, zip, concat, forkJoin, fromEvent } from 'rxjs';
...
import { mergeMap, map, concatAll, concatMapTo, debounceTime, tap } from 'rxjs/operators';
...
export class PanelScaleplateComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.windowSizeChange$ = fromEvent(window, 'resize').pipe(
			tap(() => {
		...
		this.panelTransformMatricChangeRX$ = this.panelExtendService.transformMatrixModel.valueChange$.pipe(
			debounceTime(1),
			tap(() => {
				this.panelExtendService.launchRecordPanelInfoRect$.next()
			})
		...
		this.originBackRX$ = this.panelScaleplateService.launchOrigin$.pipe(
			tap(()=>{
	 ...
	 * 绘制线条
	 */
	public drawLine(rule: CanvasRenderingContext2D, move: { moveStart: number, moveEnd: number }, line: { lineStart: number, lineEnd: number }, color: string): void {
		rule.beginPath()
	...
	public createHRulerRect(): void {
		...
		const _handle_draw_line = (i: number)=>{
			this.drawLine(_h, { moveStart: i * 10, moveEnd: i % 10 == 0 ? 0 : 10 }, { lineStart: i * 10, lineEnd: 16 }, '#ccc')
			this.drawLine(_h, { moveStart: i * 10 + 1, moveEnd: i % 10 == 0 ? 0 : 10 }, { lineStart: i * 10 + 1, lineEnd: 16 }, '#f9fafb')
			if (i % 10 == 0) {
	...
	public createVRulerRect(): void {
		...
		_v.lineWidth = 2;
		const _handle_draw_line = (i: number) => {
			this.drawLine(_v, { moveStart: i % 10 == 0 ? 0 : 10, moveEnd: i * 10 }, { lineStart: 16, lineEnd: i * 10 }, '#ccc')
			this.drawLine(_v, { moveStart: i % 10 == 0 ? 0 : 10, moveEnd: i * 10 - 1 }, { lineStart: 16, lineEnd: i * 10 - 1 }, '#f9fafb')
			if (i % 10 == 0) {
				_text_v.save();
	 ...
	 * 接受canvas面板鼠标移入
	 */
	public acceptCanvasMouseEnter( event: MouseEvent, type: 'h' | 'v' ): void {
		if (this.mouseMove$) this.mouseMove$.unsubscribe()
		const _tem_line = new LineModel(type);
		this.mouseMove$ = fromEvent(type == 'h' ? this.hRuler : this.vRuler, 'mousemove').subscribe((move: MouseEvent) => {
			this.zone.run(() => {
				if (type == 'h') {
	...
	public calcAllHVLineDrag(): void {
		...
		if( Array.isArray(_v_line) ) {
			_v_line.forEach(_v => { _v.setInPanelNum(_v.inCanvasNum + this.panelInfoModel.top - 66) })
		};
		if (Array.isArray(_h_line)) {
			_h_line.forEach(_h => { _h.setInPanelNum(_h.inCanvasNum + this.panelInfoModel.left - 216) })
		};
	}
