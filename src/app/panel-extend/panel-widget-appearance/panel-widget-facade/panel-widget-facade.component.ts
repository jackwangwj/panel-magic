...
export class PanelWidgetFacadeComponent implements OnInit {
	...
	constructor(
		...
		this.panelFacadeValueChangeRX$ = this.panelFacade.valueChange$
	...
	ngOnDestroy() {
		if (this.panelFacadeValueChangeRX$)
			this.panelFacadeValueChangeRX$.unsubscribe()
	}
	...
	public handleOnesInsetWidgetFacade(): void {
		...
		let _all_style = {
			...
			'line-height': _text.styleContent(
				_w_height -
					(_facade.borderStyle != 'none'
						? _facade.borderNumber * 2
						: 0)
			)['line-height']
		}
	...
	public quickSquare(): void {
		...
		let _widget = this.widget
		const _max = Math.max(
			_widget.conventionSiteModel.height,
			_widget.conventionSiteModel.width
		)
	...
	public quickRound(): void {
		...
		_widget.addStyleToUltimatelyStyle({
			width: `${_max}px`,
			height: `${_max}px`,
			'border-radius': _widget.panelFacadeModel.styleContent()[
				'border-radius'
			]
		})
		_widget.profileModel.setData({
