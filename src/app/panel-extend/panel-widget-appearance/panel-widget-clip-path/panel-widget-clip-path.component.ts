...
import { TPolygonType, ClipPointModel, ClipInsetModel, ProfileModel, ClipPolygonModel, ClipCircleModel, ClipEllipseModel, TClipPathType } from '../../panel-scope-enchantment/Model';
...
import { interceptionNumber } from '@ng-public/util';
...
export class PanelWidgetClipPathComponent implements OnInit {
	...
	constructor(
		private clipPathResizeMaskService: ClipPathResizeMaskService,
		...
		this.profileOuterSphereRX$ = this.panelScopeEnchantmentService.scopeEnchantmentModel.profileOuterSphere$.subscribe(pro=>{
			if( pro ) {
				if (this.profileTensileMoveRX$) this.profileTensileMoveRX$.unsubscribe()
				this.profileTensileMoveRX$ = pro.valueChange$.pipe(
					debounceTime(1)
				).subscribe(value=>{
					if (this.clipPathResizeMaskService.clipPathMaskModel.currentPathType$.value == 'circle'
						&& this.clipCirclePath ) {
	...
	public calcPer(drag: DraggablePort, targ: number, type: 'width' | 'height' = 'width'): number {
		const _ = (((targ / 100) * this.widgetProfile[type] + drag[type == 'width' ? 'left' : 'top']) / this.widgetProfile[type]) * 100
	...
	public handleInsetClipDragPoint( drag: DraggablePort, point: ClipPointModel ): void {
		const _math_obj = {
			'inset-l': ()=>{
				...
				this.clipInsetPath.insetT.left = this.clipInsetPath.insetL.left + (this.clipInsetPath.insetR.left - this.clipInsetPath.insetL.left) / 2
			...
			'inset-t': () => {
				...
				this.clipInsetPath.insetL.top = this.clipInsetPath.insetT.top + (this.clipInsetPath.insetB.top - this.clipInsetPath.insetT.top) / 2
			...
			'inset-c': () => {
				...
				const _is_side_v = (): boolean => {
					return this.calcPer(drag,this.clipInsetPath.insetL.left) == 0 || this.calcPer(drag,this.clipInsetPath.insetR.left) == 100
				}
				// 验证纵向是否已达到边缘，是的话则不允许上下移动
				const _is_side_h = (): boolean => {
					return this.calcPer(drag,this.clipInsetPath.insetT.top, 'height') == 0 || this.calcPer(drag,this.clipInsetPath.insetB.top, 'height') == 100
				}
				if (!_is_side_v() ) {
