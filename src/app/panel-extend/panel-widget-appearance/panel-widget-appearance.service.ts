...
import { AppearanceModel, ConventionSiteModel, PanelTextModel, PanelFacadeModel, PanelFilterModel, alignType, PanelAnimationModel, animationLabelOptions } from './Model';
import { BehaviorSubject, Subject } from 'rxjs';
import { PanelShadowModel } from './Model/PanelShadowModel';
...
export class PanelWidgetAppearanceService {
	...
	// 外观设置组件的样式数据
	public conventionSiteModel$: BehaviorSubject<ConventionSiteModel> = new BehaviorSubject<ConventionSiteModel>(new ConventionSiteModel())
	// 通用的文本设置组件的数据模型
	public panelTextModel$: BehaviorSubject<PanelTextModel> = new BehaviorSubject<PanelTextModel>(new PanelTextModel())
	// 通用的外观设置组件的数据模型
	public panelFacadeModel$: BehaviorSubject<PanelFacadeModel> = new BehaviorSubject<PanelFacadeModel>(new PanelFacadeModel())
	// 通用的阴影设置组件的数据模型
	public panelShadowModel$: BehaviorSubject<PanelShadowModel> = new BehaviorSubject<PanelShadowModel>(new PanelShadowModel())
	// 通用的滤镜设置组件的数据模型
	public panelFilterModel$: BehaviorSubject<PanelFilterModel> = new BehaviorSubject<PanelFilterModel>(new PanelFilterModel())
	// 通用的动效设置组件的数据模型
