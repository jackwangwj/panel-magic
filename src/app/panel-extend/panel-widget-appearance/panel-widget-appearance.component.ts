...
export class PanelWidgetAppearanceComponent implements OnInit {
	...
	@ViewChild('otherSiteContainer', { read: ViewContainerRef }) public otherSiteContainer: ViewContainerRef

	constructor(
		private componentFactoryResolver: ComponentFactoryResolver,
		private panelWidgetAppearanceService: PanelWidgetAppearanceService,
		private panelWidgetDetailsSiteService: PanelWidgetDetailsSiteService,
		private panelScopeEnchantmentService: PanelScopeEnchantmentService,
		private panelExtendService: PanelExtendService
	) {
		this.profileOuterSphereRX$ = this.scopeEnchantment.profileOuterSphere$.pipe(
	...
	public subPruOuterSphereValueChange(sphere: OuterSphereHasAuxlModel ): void {
		if (sphere ) {
			if (this.profileValueChangeRX$) this.profileValueChangeRX$.unsubscribe();
			const _inset_widget = this.scopeEnchantment.outerSphereInsetWidgetList$.value;
			this.profileValueChangeRX$ = sphere.valueChange$
				.pipe(
					auditTime(10)
				).subscribe(value => {
					if (_inset_widget.length == 1) {
						let _con_obj = cloneDeep(value)
						this.conventionSite.setData(_con_obj)
					}else {
						this.handleSetMoreWidget()
					}
				})
	...
	public alignWay(type: alignType ): void {
		...
		const _repeat_pro = (widget_list: Array<PanelWidgetModel>) => {
			this.scopeEnchantment.outerSphereInsetWidgetList$.next(widget_list)
		...
		const _calc_profile = (e: PanelWidgetModel,pro: OuterSphereHasAuxlModel | ProfileModel): void=>{
			...
			if( e.profileModel.rotate != 0 ) _offset_coord = this.panelScopeEnchantmentService.handleOuterSphereRotateOffsetCoord(e.profileModel);
			const _config = {
			...
			if (_config[type] != undefined ) e.profileModel[(type == 'left' || type == 'right' || type =='center')?'left':'top'] = _config[type];
		};
		let _equidistance_fn = (type: 'width' | 'height') => {
			...
			const _sort_list = _widget_list.sort((a, b) => a.profileModel[type == 'width' ? 'left' : 'top'] - b.profileModel[type == 'width' ? 'left' : 'top']);
			_sort_list.forEach((_w, _i) => {
				if (_i > 0) {
					_w.profileModel[type == 'width' ? 'left' : 'top'] = _widget_list[_i - 1].profileModel[type == 'width' ? 'left' : 'top'] + _widget_list[_i - 1].profileModel[type] + _space_bet
				}
			})
		};
		if (type != 'crosswiseEquidistance' && type != 'verticalEquidistance') {
			_widget_list.forEach(_e => {
				_calc_profile(_e, _widget_list.length > 1 ? _pro : <ProfileModel>{
					left: 0,
	...
	public handleOnlyWidgetStyleContent(): void {
		...
		_inset_widget.profileModel.setMouseCoord([
			_inset_widget.profileModel.left,
			_inset_widget.profileModel.top
		])
	...
	public appearanceDownEvent(): void {
		this.panelWidgetAppearanceService.appearanceModel.zIndex = this.panelWidgetDetailsSiteService.detailsModel.zIndex + 1
	}
