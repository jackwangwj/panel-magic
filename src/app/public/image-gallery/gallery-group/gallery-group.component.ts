...
export class GalleryGroupComponent implements OnInit {
	...
	public popEditGroupAffirm(data: ImageGroupModel): void {
		...
		if( data.name.trim() == '' ) {
			...
			this.galleryGroupService.editPictureGroup( data.id, data.name).subscribe(b=>{
				if( b ) {
	...
	public createGroup(): void {
		...
		this.galleryGroupService.createPictureGroup().subscribe(b=>{
	...
	public deleteGroup( data: ImageGroupModel ): void {
		const _modal = this.nzModalService.confirm({
			...
			nzOnOk: () => {
				...
				this.galleryGroupService.delPictureGroup(data.id).subscribe(b => {
